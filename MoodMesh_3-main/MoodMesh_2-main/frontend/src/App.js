...
import { Brain, MessageCircle, Star, Sparkles, Heart, Users, Send, LogOut, Bot } from "lucide-react";
...
const Home = () => {
  ...
  const handleGetStarted = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${API}/profile/create?username=${encodeURIComponent(username)}`);
      localStorage.setItem("moodmesh_user", JSON.stringify(response.data));
      toast.success("Welcome to MoodMesh!");
      navigate("/dashboard");
    } catch (error) {
        ...
        <div className="text-center mb-12">
          <div className="flex items-center justify-center mb-6">
            <div className="relative">
              <Brain className="w-20 h-20 text-teal-600" strokeWidth={1.5} />
              <Sparkles className="w-8 h-8 text-yellow-500 absolute -top-2 -right-2 animate-pulse" />
            </div>
          </div>
          <h1 className="text-5xl lg:text-6xl font-bold text-gray-900 mb-4" style={{ fontFamily: 'EB Garamond, serif' }}>
            MoodMesh
          </h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto" style={{ fontFamily: 'Inter, sans-serif' }}>
            Your personalized mental wellness companion. Log your mood, receive AI-powered coping strategies, 
            and connect with a supportive community.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 mb-12">
          <Card className="border-2 hover:border-teal-300 transition-all duration-300 hover:shadow-lg" data-testid="feature-mood-logging">
            <CardHeader>
              <Brain className="w-10 h-10 text-teal-600 mb-2" />
              <CardTitle className="text-xl">Mood Logging</CardTitle>
            </CardHeader>
            <CardContent>
              <CardDescription>Express how you feel and track your emotional journey over time</CardDescription>
            </CardContent>
          </Card>

          <Card className="border-2 hover:border-teal-300 transition-all duration-300 hover:shadow-lg" data-testid="feature-ai-support">
            <CardHeader>
              <Sparkles className="w-10 h-10 text-yellow-500 mb-2" />
              <CardTitle className="text-xl">AI Coping Strategies</CardTitle>
            </CardHeader>
            <CardContent>
              <CardDescription>Get personalized mental health recommendations powered by AI</CardDescription>
            </CardContent>
          </Card>

          <Card className="border-2 hover:border-teal-300 transition-all duration-300 hover:shadow-lg" data-testid="feature-community">
            <CardHeader>
              <Users className="w-10 h-10 text-blue-600 mb-2" />
              <CardTitle className="text-xl">Mood Rooms</CardTitle>
            </CardHeader>
            <CardContent>
              <CardDescription>Join supportive group chats and connect with others who understand</CardDescription>
            </CardContent>
          </Card>
        </div>

        <Card className="max-w-md mx-auto border-2 border-teal-200 shadow-xl" data-testid="welcome-card">
          <CardHeader>
            <CardTitle className="text-2xl text-center">Get Started</CardTitle>
            <CardDescription className="text-center">Enter your name to begin your wellness journey</CardDescription>
          </CardHeader>
          <CardContent>
